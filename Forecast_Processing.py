...
def concat2(ALL,pareto):
    ...
    group = group.drop(['ItemNumber','Max',"6mAVG","3mAVG","BackOrder_Qty","Ordered_Qty","soh","RBU_ItemNumber","RBU"], axis=1)
    group = group.drop(['ByPass', 'Current','Curr+1','Curr+2','Curr+3','Curr+4','Curr+5'],axis=1)
...
for i in range(a):
    with alive_bar(i, title="Forecasting", theme='smooth') as bar:
        ...
        if message == False:
            ...
            soh,orders,message = prepare_files(soh,orders)
            wo = wo_prep(wo)
            if message == False:
